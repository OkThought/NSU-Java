...
public class StatisticsCollectorTest {
	...
	public void custom() throws Exception {
		...
		assertEquals(2, statistics.getFilterFileStatisticsMap().get(filters[2]).fileCount); // |(.cpp .java)
		assertEquals(7, statistics.getFilterFileStatisticsMap().get(filters[2]).lineCount);
		assertEquals(0, statistics.getFilterFileStatisticsMap().get(filters[3]).fileCount); // &(.cpp .java)
